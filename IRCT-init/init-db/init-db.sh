...
{
    mysql --host=${IRCTMYSQLADDRESS} --user=${user} --password=${IRCTMYSQLPASS} ${db}
} || {
    exit $?
}
...
{
    # TODO: Check if database is already populated (prevent duplicate entries)
    ...
    if [ "x${S3_BUCKET_NAME}" != x ]; then
        ...
        echo "confirm S3 Bucket populated"
        mysql --host=${IRCTMYSQLADDRESS} --user=${user} --password=${IRCTMYSQLPASS} ${db}  -e \
